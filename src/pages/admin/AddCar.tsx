...
export default function AddCar({ initial }: { initial?: any } = {}) {
  const nav = useNavigate();
  const isEdit = !!initial;
  return (
    <div className="space-y-6">
      <Link to="/admin/cars" className="inline-flex items-center gap-1 text-sm text-muted-foreground hover:text-foreground">
      ...
      <div className="flex flex-wrap items-center justify-between gap-3">
        ...
        <div className="flex gap-2">
          ...
          <Button className="bg-primary hover:bg-primary-glow" onClick={() => { toast.success(isEdit ? "Car updated" : "Car published"); nav("/admin/cars"); }}>{isEdit ? "Update" : "Publish car"}</Button>
        </div>
      </div>
